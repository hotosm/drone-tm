...
async def identify_flight_gaps(
    ...
    Args:
        db (Connection): Asynchronous database connection (psycopg).
        project_id (UUID): Unique identifier for the project.
        batch_id (UUID): Unique identifier for the specific upload batch.
        task_id (UUID): Unique identifier for the task boundary.

    Returns:
        Optional[bytes]: The binary content of a generated flight plan if gaps are detected and reconstructed.
    """
    ...
    for idx, segment in enumerate(segments):
        ...
        try:
            drone_model_enum = db_drone_model.upper().replace(" ", "_")
            drone_type_arg = DroneType(drone_model_enum)
        except ValueError:
            log.warning(f"Unknown drone model '{db_drone_model}'.")
        ...
        if side_dist_moved_legs_list:
            ...
            specs = DRONE_PARAMS[drone_type_arg]
            avg_alt = np.median(
        ...
        log.debug(
            f"Global side overlap median for all legs: {global_side_overlap_median}"
        )
        ...
        for idx, leg in enumerate(segment_legs):
            ...
            if previous_leg is not None and global_side_overlap_median:
                ...
                if side_overlap_dist > (
                    global_side_overlap_median * GAP_EXCEED_BASELINE
                ):
                    potential_missing_images.append(
                        {
        ...
        if len(potential_missing_images) > MIN_GAP_IMAGES:
            ...
            if task_row and task_row["geometry"]:
                ...
                confirmed_gaps = _validate_gaps_in_task_aoi(
                    task_aoi_outline, potential_missing_images
                )
                ...
                if confirmed_gaps:
                    ...
                    return _generate_reconstruction_flightplan(
                        confirmed_gaps,
                        task_aoi_outline,

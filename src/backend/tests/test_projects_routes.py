...
@pytest.mark.asyncio
async def test_odm_queue_info_surfaces_finalizing_when_scaleodm_40_but_s3_missing(
    client, app, db, auth_user, create_test_project, monkeypatch
):
    """ScaleODM reports 40 but the per-task COG isn't in S3 yet.

    Refresh no longer enqueues finalize_scaleodm_task as a "secondary
    trigger" - it surfaces the task as Finalizing and waits for the
    next Refresh tick's HEAD to reconcile inline.
    """
    project_id = create_test_project
    task_id = str(uuid.uuid4())
    odm_task_uuid = str(uuid.uuid4())
    ...
    monkeypatch.setattr(project_routes.aiohttp, "ClientSession", FakeSession)
    # S3 HEAD definitively misses - test the "ScaleODM 40 but file not yet
    # in S3" path. project_logic.check_task_ortho_in_s3 calls s3_object_exists
    # via asyncio.to_thread, so patching the underlying sync function is enough.
    monkeypatch.setattr(project_logic, "s3_object_exists", lambda *_a, **_kw: False)
    app.dependency_overrides[get_redis_pool] = lambda: fake_redis

    response = await client.get(f"/api/projects/odm/queue-info/{project_id}/")

    assert response.status_code == 200
    # No background work is enqueued in arq: Refresh no longer chains
    # finalize_scaleodm_task as a secondary trigger.
    assert fake_redis.jobs == []

    body = response.json()
    assert body["total_running"] == 1
    assert body["total_failed"] == 0
    # The task surfaces in the running group with the "Finalizing" label.
    running_groups = [g for g in body["groups"] if g["status_code"] == 20]
    assert len(running_groups) == 1
    finalizing = next(
        (t for t in running_groups[0]["tasks"] if t["status_label"] == "Finalizing"),
        None,
    )
    assert finalizing is not None
    assert finalizing["dtm_task_id"] == task_id


async def _insert_started_task(
    db,
    *,
    project_id: str,
    user_id: str,
    ...
    """Insert a task in IMAGE_PROCESSING_STARTED for queue-info testing."""
    task_id = str(uuid.uuid4())
    outline = json.dumps(
        {
            "type": "Polygon",
    ...
    async with db.cursor() as cur:
        await cur.execute(
            """
            INSERT INTO tasks (id, project_id, project_task_index, outline, odm_task_uuid)
            VALUES (%s, %s, %s, ST_SetSRID(ST_GeomFromGeoJSON(%s), 4326), %s)
            """,
            (task_id, project_id, task_index, outline, odm_task_uuid),
        )
        await cur.execute(
            """
            INSERT INTO task_events (
                event_id, project_id, task_id, user_id, state, comment,
...
@pytest.mark.asyncio
async def test_odm_queue_info_finalises_task_inline_when_s3_hit(
    client, app, db, auth_user, create_test_project, monkeypatch
):
    """Happy path: S3 HEAD finds the COG ⇒ transition inline + schedule ScaleODM cleanup."""
    project_id = create_test_project
    odm_task_uuid = str(uuid.uuid4())
    task_id = await _insert_started_task(
        db,
    ...
    # S3 says the file is present.
    monkeypatch.setattr(project_logic, "s3_object_exists", lambda *_a, **_kw: True)
    # Track that ScaleODM cleanup is *scheduled* (via BackgroundTasks) and
    # eventually invoked - record the kwargs the route passes us.
    cleanup_calls = []

    async def fake_remove_scaleodm_task(*, scaleodm_url, odm_task_uuid):
        cleanup_calls.append({"url": scaleodm_url, "uuid": odm_task_uuid})

    monkeypatch.setattr(
        project_routes, "remove_scaleodm_task", fake_remove_scaleodm_task
    )
    ...
    assert response.status_code == 200
    body = response.json()
    # Task should no longer be in the running queue - we transitioned it
    # FINISHED inline, so it's dropped from the response groups.
    assert body["total_running"] == 0
    assert body["total_tasks"] == 0

    # DB confirms IMAGE_PROCESSING_FINISHED + assets_url set.
    async with db.cursor() as cur:
        await cur.execute(
            """
            SELECT te.state, t.assets_url
            FROM tasks t
    ...
    assert row[0] == "IMAGE_PROCESSING_FINISHED"
    assert row[1] == f"projects/{project_id}/{task_id}/odm/"

    # ScaleODM cleanup ran in BackgroundTasks (httpx ASGITransport drains
    # BackgroundTasks before returning the response in tests).
    assert cleanup_calls == [
        {
            "url": project_routes.settings.ODM_ENDPOINT,
            "uuid": odm_task_uuid,
        }
    ]
...
@pytest.mark.asyncio
async def test_odm_queue_info_keeps_task_visible_when_db_transition_fails(
    client, app, db, auth_user, create_test_project, monkeypatch
):
    """S3 hit but the DB transition raises ⇒ task stays visible, no cleanup."""
    project_id = create_test_project
    odm_task_uuid = str(uuid.uuid4())
    task_id = await _insert_started_task(
        db,
    ...
    # Force finalise_task_in_db to fail by making update_task_state_system raise.
    async def boom(**_kwargs):
        raise RuntimeError("simulated DB failure")

    monkeypatch.setattr(project_logic.task_logic, "update_task_state_system", boom)
    ...
    # Make the ScaleODM /info call return something innocuous so the rest
    # of get_odm_queue_info doesn't crash hunting for live progress.
    class FakeResponse:
        status = 200

        async def json(self):
            return {"status": {"code": 20}}
    ...
    assert response.status_code == 200
    body = response.json()
    # Task must remain visible so the user can retry on the next Refresh.
    assert body["total_running"] == 1
    running_groups = [g for g in body["groups"] if g["status_code"] == 20]
    assert any(
        t["dtm_task_id"] == task_id for grp in running_groups for t in grp["tasks"]
    )

    # No ScaleODM cleanup must have been scheduled for a failed transition.
    assert cleanup_calls == []

...
describe('MapSection', () => {
  ...
  describe('handleGcpSelection (table row click)', () => {
    it('should show popup and fly to feature when GCP is found', () => {
      ...
      expect(mockMap.flyTo).toHaveBeenCalledWith(
        expect.objectContaining({ center: [85.3, 27.7] })
      );
    });
  ...
  describe('addBaseLayers', () => {
    it('should add satellite, topo, and hybrid sources and layers', () => {
      ...
      expect(mockMap.addSource).toHaveBeenCalledWith('satellite', expect.objectContaining({ type: 'raster' }));
      expect(mockMap.addSource).toHaveBeenCalledWith('topo', expect.objectContaining({ type: 'raster' }));
      expect(mockMap.addSource).toHaveBeenCalledWith('hybrid', expect.objectContaining({ type: 'raster' }));
  ...
  describe('setupClickHandler', () => {
    it('should register click and mouse events on the map', () => {
      ...
      expect(mockMap.on.mock.calls.some((c: any[]) => c[0] === 'click' && typeof c[1] === 'function')).toBe(true);

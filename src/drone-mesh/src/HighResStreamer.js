...
// GLB parsing / texture stripping / tile decode live in ./glb.js (shared with
// the single-GLB base-mesh builder).
...
export class HighResStreamer {
  ...
  async load(url, lowResObject) {
    ...
    this.tiles = meshes.map((mesh, i) => {
      const matIndex = materialIndexOf(mesh, i);
  ...
  async decodeToImage(bytes, mime, maxSize) {
    this.decodeCount++;
    return decodeTileImage(bytes, mime, maxSize);
  }

  // --- GLB surgery (parseGLB / extractTileImages / stripTextures /
  // rebuildGLB / materialIndexOf) now lives in ./glb.js. ---------------------

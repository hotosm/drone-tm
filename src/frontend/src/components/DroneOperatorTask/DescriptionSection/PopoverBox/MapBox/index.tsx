...
const ImageMapBox = () => {
  ...
  const [selectedPointImageName, setSelectedPointImageName] = useState<string[]>([]);
  const [progressBar, setProgressBar] = useState(false);
  const [loadingWidth, setLoadingWidth] = useState(0);
  const [imageFilesGeoJsonData, setImageFilesGeoJsonData] = useState<Record<string, any>>();
  const [imageFilesLineStringData, setImageFilesLineStringData] = useState<Record<string, any>>();
  const [popupData, setPopupData] = useState<Record<string, any>>();

  const uploadedImageType = useTypedSelector((state) => state.droneOperatorTask.uploadedImagesType);
  const filesExifData = useTypedSelector((state: any) => state.droneOperatorTask.filesExifData);
  ...
  const taskAreaPolygon = useTypedSelector((state) => state.droneOperatorTask.taskAreaPolygon);
    ...
    return (
      <div className="naxatw-flex naxatw-flex-col naxatw-items-start naxatw-gap-2">
        <div className="naxatw-flex naxatw-flex-col naxatw-gap-1">
          <p className="naxatw-text-base naxatw-font-medium naxatw-text-black">{popupData?.name}</p>
          <p>
  ...
  const { mutate } = useMutation({
    mutationFn: async (data: any) => {
      const urlsData = await getImageUploadLink(uploadedImageType === "replace", data);
  ...
  const pointsInsideTaskArea = useMemo(() => {
    ...
    filesExifData?.forEach((element: any) => {
      const imagePoint = point([element.coordinates.longitude, element.coordinates.latitude]);
        ...
        <div className="naxatw-h-[calc(100vh-250px)] naxatw-w-full naxatw-bg-gray-200">
          ...
          >
            ...
            <VectorLayer
              ...
              onFeatureSelect={(data) => {
                setSelectedPointImageName((prev) => {
                  ...
                  map?.setPaintProperty("image-points-map-layer", "circle-stroke-color", [
                    "case",
                ...
                <Button
                  ...
                  onClick={() => {
                    const newFilesData = filesExifData.filter(
                      (filex: any) => !selectedPointImageName.includes(filex.file.name),
                    );
            ...
            {!activeSelection && (
              <AsyncPopup
                map={map as Map}
                showPopup={(feature: Record<string, any>) => feature?.source === "image-points-map"}
                popupUI={getPopupUI}
          ...
          <p className="naxatw-text-lg naxatw-font-medium naxatw-text-yellow-600">
            {Number(100 - ((pointsInsideTaskArea / filesExifData.length) * 100 || 0)).toFixed(0)}%

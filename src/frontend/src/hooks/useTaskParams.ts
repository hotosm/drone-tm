...
export default function useTaskParams() {
  ...
  const { data: taskDataByIndex, isFetching: isTaskByIndexFetching } =
  ...
  const { data: taskDataById, isFetching: isTaskByIdFetching } =
    useGetIndividualTaskQuery(urlTaskId as string, {
  ...
  return {
    ...
    taskData,
    isResolving:
      isProjectFetching || isTaskByIndexFetching || isTaskByIdFetching,
  };
}
